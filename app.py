...
documents = sorted(set(c["document"] for c in chunks if "document" in c))
    ...
    if request.method == "POST":
        if request.form.get("clear"):
            return render_template("index.html", answer=[], question="", documents=documents,
                                   selected_doc="", refine_query="", use_semantic=False)
        ...
        filtered = [c for c in chunks if
                    (not selected_doc or c["document"] == selected_doc) and
